            ...
            if user is not None:
                if user.is_active:
                    try:
                        ...
                        return Response({'detail': 'Invalid email or password jiji '}, status=status.HTTP_400_BAD_REQUEST)
                    login(request, user)
...
class ProfileAPIView(ListAPIView):
    ...
    def get(self, request, id):
        try:
            ...
            print(user)
            return Response({'user':{ **serializer.data, **serializer2.data}}, status=status.HTTP_200_OK)
        except: 
            return Response({'detail': serializer.error}, status=status.HTTP_400_BAD_REQUEST)
